readCqcItems0<-function(file,ncats,itemmeanzero=FALSE){
  ms<-ncats-1
  row<-length(ms)
  col<-max(ms)
  ...
  for (i in 1:row) {
    m<-sum(!is.na(form[i,]))
    if (m==1) {
      tmp<-list(name=paste("V",i,sep=""),nfac=1,itemtype=2,ncat=m+1,a=1,ck=-form[i,1:m],bk=form[i,1:m])
      class(tmp)<-"uirt"
      res[[i]]<-tmp
    } else {
      tmp<-list(name=paste("V",i,sep=""),nfac=1,itemtype=3,ncat=m+1,gpc=1,astar=1,as=0:m,ak=0:m,
                ck=c(0,-cumsum(form[i,(1:m)])),bk=c(0,cumsum(form[i,(1:m)])))
      class(tmp)<-"uirt"
      res[[i]]<-tmp
    }
  }
